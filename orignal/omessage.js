...
async function fetchUsers() {
    try {
        ...
        const response = await fetch('http://ontheway.runasp.net/GetAll', {
            method: 'GET',
            headers: {
        ...
        users.forEach(user => {
            ...
            chatItem.setAttribute('onclick', `openChat('${user.userName || 'User'}', '${user.id}')`);

            const chatImage = document.createElement('img');
            chatImage.src = user.imageUrl || 'https://via.placeholder.com/40';
            chatImage.alt = `${user.userName || 'User'} Photo`;
...
async function fetchMessages(userId) {
    try {
        ...
        const token = getAuthToken();
        const response = await fetch('http://ontheway.runasp.net/api/Chat/messages', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': '*/*'
...
async function sendMessage(userId, message) {
    ...
    try {
        ...
        const token = getAuthToken();
        const response = await fetch('http://ontheway.runasp.net/api/Chat/send/admin-to-user', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
